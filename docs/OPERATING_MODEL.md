...
- Runtime binary comes from `UBL-MULTI` (`contracts/VERSIONS.lock`).
- Contract compatibility is pinned against `UBL-CORE` (`contracts/VERSIONS.lock`).
- This repo owns only config, process lifecycle (PM2), deployment scripts, and validation.

## Update Flow

1. Change `MULTI_REF` and/or `CORE_REF` in `contracts/VERSIONS.lock`.
